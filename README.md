...
日常で頻繁に使う小さなツール（価格計算・メモ・ファイルアップロード・ブラウザ互換性検索など）を1つのダッシュボードにまとめたWebアプリです。<br/>
AIエージェントを活用した安全な開発やCI/CDによる自動化など、プロジェクト運用の検証を目的としています。
...
AIエージェントで自律的に開発を進めるにあたり、開発環境・AIエージェント・運用フローの3つの点から安全性を担保しています。

### 開発環境の隔離

ホストPCの安全のため、Dockerコンテナ内で開発を行っています。
...
- **ホストのマウントをプロジェクトディレクトリに限定** — プロジェクト外のホストファイルへのアクセス・改ざんを防ぐ
- **SSHキーを読み取り専用でマウント** — SSHキーの書き換え・削除を防止
- **Docker socketは未マウント** — コンテナ内から新規コンテナの起動やホスト操作を行う典型的な脱出経路を塞ぐ

### AIエージェントの権限制御

コンテナの中でも、設定ファイルやhook機能を用いてAIエージェントが直接実行できる操作を制限しています。
...
- **PR作成前に確認を必須化** — PR作成をきっかけにCIチェックが走り、通過すればマージ・本番デプロイ（GitHub Actionsによる）まで一連の流れが自動で進むため、意図しない変更が確認なしにそのまま本番まで到達してしまうことを防ぐ
- **新規パッケージ追加前に確認を必須化** — 脆弱性のあるパッケージやtyposquatting（タイプミスを狙った偽パッケージ）などを、気づかないまま導入してしまうことを防ぐ
- **DBマイグレーション実行前に確認を必須化** — AIの誤った判断で、データやスキーマが意図せず変更されることを防ぐ

### 運用フローの保護

AIエージェントが制御をすり抜けた場合に備え、mainブランチを保護しています。

- **Branch protection rulesでmainブランチへの直接push・force pushを禁止** — AIエージェント側の制御が万が一機能しなくても、mainブランチへの直接反映や履歴の破壊を防ぐ
...
運用や機能拡張を続けて問題がなければ、現在、手動で管理をしている[多機能生産性管理Webアプリ](https://github.com/aoto-me/scheduler_public)の開発・運用にもAIエージェントとCI/CDによる自動化を導入したいと考えています。<br/>
本プロジェクトで検証した仕組みを、他のプロジェクトへ適用していく予定です。
